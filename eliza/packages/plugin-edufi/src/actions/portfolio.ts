...
export const portfolio: Action = {
    name: "SHOW_PORTFOLIO",
    description: "Show your EDU wallet portfolio with token balances and USD values",
    examples: [
        [
            {
                user: "user1",
            ...
            },
            {
                user: "assistant",
                content: {
                    text: "Fetching your current portfolio data from EDU Network...",
                },
            },
        ],
    ],
    ...
    handler: async (runtime, message: Memory, state: State | undefined, options, callback) => {
        ...
        try {
            ...
            const tokenBalances: TokenBalance[] = [{
                symbol: 'EDU',
                name: TOKENS.EDU.name,
                balance: nativeBalanceFormatted,
                usdPrice: pricesResponse.data?.[TOKENS.EDU.coingeckoId]?.usd || 0,
                usdValue: Number(nativeBalanceFormatted) * (pricesResponse.data?.[TOKENS.EDU.coingeckoId]?.usd || 0),
            }];
    ...
    similes: [
        ...
        "like viewing your investment portfolio on EDU",
    ],
};
