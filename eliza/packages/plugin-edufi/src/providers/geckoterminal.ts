...
export const geckoTerminalProvider: Provider = {
    get: async (runtime: IAgentRuntime, message: Memory, _state?: State): Promise<string | null> => {
        elizaLogger.info("geckoTerminalProvider::get");
        ...
        try {
            const response = await fetch(
                `${GECKO_TERMINAL_API}/networks/${NETWORK_ID}/pools`,
                {
                    headers: {
            ...
            if (!data.data?.length) {
                return "No pools found on Edu network.";
            }
            ...
            return `# Edu Network Pool Analysis
