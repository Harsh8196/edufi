...
const TRACKED_TOKENS = [
  ...
  { id: 'blend-3', symbol: 'blend', name: 'Blend' },
  { id: 'edu-chain-bridged-usdt-edu-chain', symbol: 'usdt', name: 'USDT' },
  { id: 'edu-chain-bridged-usdc-edu-chain', symbol: 'usdc', name: 'USDC' },
  { id: 'edu-stabledollar', symbol: 'eds', name: 'EDU Stable Dollar' },
  { id: 'wrapped-edu', symbol: 'wedu', name: 'Wrapped EDU' }
];
...
export const coinGeckoProvider: Provider = {
  async get(_runtime: IAgentRuntime, message: Memory, _state?: State): Promise<string> {
    elizaLogger.info("marketAnalysisProvider::get");
    try {
      let selectedTokens = TRACKED_TOKENS;
      const requestedTokens = message.content?.text?.toLowerCase().match(/\b(btc|eth|usdt|blend|esd|wedu|usdc)\b/g);
